...
def run_exp(epoches, batch_size, subject_num, model_type, cuda, single_subject, single_subject_num):
    ...
    exp.run()
    # th.save(model, "models\{}-trialwise-subject{}-{}sec-{}epoches-torch_model".format(model_type, single_subject_num, ((trial_length - 1) / 256), epoches))
    return exp
...
if __name__ == '__main__':
    ...
    np.save("{}-trialwise-subject{}-2.5sec-{}epoches".format(model, single_subject_num, max_epochs), exp.epochs_df.iloc[:])
