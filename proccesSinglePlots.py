            ...
            try:
                ...
                if subject ==1 :        
                    if plotType == 'accuracy':
                        plt.plot((1- subjectData[:,3]), color=colors['skyblue'], linestyle='dashed', linewidth=0.5, label='Single subjects train accuracy')
                        plt.plot((1- subjectData[:,4]), color=colors['peachpuff'], linestyle='dashed', linewidth=0.5, label='Single subjects validation accuracy')
                        if withTest == True:
                            plt.plot((1- subjectData[:,5]), color=colors['lightgreen'], linestyle='dashed', linewidth=0.5, label='Single subjects test accuracy')
                    else:
                        plt.plot(subjectData[:,3], color=colors['skyblue'], linestyle='dashed', linewidth=0.5, label='Single subjects train accuracy')
                        plt.plot(subjectData[:,4], color=colors['peachpuff'], linestyle='dashed', linewidth=0.5, label='Single subjects validation accuracy')
                        if withTest == True:
                            plt.plot(subjectData[:,5], color=colors['lightgreen'], linestyle='dashed', linewidth=0.5, label='Single subjects test accuracy')

                else:
                    if plotType == 'accuracy':
                        plt.plot((1- subjectData[:,3]), color=colors['skyblue'], linestyle='dashed', linewidth=0.5)
                        plt.plot((1- subjectData[:,4]), color=colors['peachpuff'], linestyle='dashed', linewidth=0.5)
                        if withTest == True:
                            plt.plot((1- subjectData[:,5]), color=colors['lightgreen'], linestyle='dashed', linewidth=0.5)

                    else:
                        plt.plot(subjectData[:,3], color=colors['skyblue'], linestyle='dashed', linewidth=0.5)
                        plt.plot(subjectData[:,4], color=colors['peachpuff'], linestyle='dashed', linewidth=0.5)
                        if withTest == True:
                            plt.plot(subjectData[:,5], color=colors['lightgreen'], linestyle='dashed', linewidth=0.5)

                for index in range(0, len(subjectData)):
                    if plotType == 'accuracy':
                        meanTrain[0, index] += 1- subjectData[index, 3]
                        meanVal[0, index] += 1- subjectData[index, 4]
                        if withTest == True:
                            meanTest[0, index] += 1- subjectData[index, 5]
                    else:
                        meanTrain[0, index] += subjectData[index, 3]
                        meanVal[0, index] += subjectData[index, 4]
                        if withTest == True:
