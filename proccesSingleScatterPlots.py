...
for shallowStrategy in trainStrategies:
    for deepStrategy in trainStrategies:
        ...
        for subject in range(1,48):
            ...
            bestShallow = 0
            try:
                accTestShallow[j] = 1- np.load("finetuning\shallow-{}-singleSubjectNum{}-2.5sec-800epoches-testSetMisclass.npy".format(shallowStrategy, subject))

                accTestDeep[j] = 1- np.load("finetuning\deep-{}-singleSubjectNum{}-2.5sec-800epoches-testSetMisclass.npy".format(deepStrategy, subject))
